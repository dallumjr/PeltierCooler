 ...
 */
PID tempController(&Celsius, &Output, &Setpoint, 25, 3.45, .25, REVERSE);
...
void loop() {
  /*Keep the fan running the entire time*/
  //analogWrite(fan, 200);
  
  /*Begin temperature monitoring */
  Celsius = getTemperature();
  boolean s = tempController.Compute();
//  if (Celsius > Setpoint) analogWrite(peltier, 255);
//  else analogWrite(peltier, Output--);
  analogWrite(peltier, Output);
  //analogWrite(8, Output); //for testing with the scope
  

//  Serial.println("Output level of Peltier");
  ...
  resetCounter++;
  //Serial.println("Peltier Information");
  //Serial.print("Power = ");
...
//  Serial.println(s);
  ...
  //delay(500); //just here to slow down the output. You can remove this
